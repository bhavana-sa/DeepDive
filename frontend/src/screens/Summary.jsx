...
export default function Summary({ session, onHome, onReplay, xpGained }) {
  ...
  const trend = session.session_progress?.mastery_trend || [];
  const latestScore = trend.length > 0 ? trend[trend.length - 1]?.score : 0;
  const score = Number(ev.score !== undefined ? ev.score : (latestScore !== undefined ? latestScore : 0));
  const passed = !!(ev.passed !== undefined ? ev.passed : (trend.length > 0 ? trend[trend.length - 1]?.passed : false));
