...
import { Sparkles, Search, ArrowRight, Leaf } from 'lucide-react';
import { TOPICS, CUSTOM_DEMO_TOPICS, EXAMPLE_CHIPS } from '../data/topics';
...
export default function Home({ onStartConcept, onStartCustom, busy, busyLabel }) {
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      ...
      {/* Science / Biology demo topics — pre-seeded, always works */}
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '4px 0 12px' }}>
          <Leaf size={18} color="var(--green)" />
          <span style={{ fontWeight: 900, fontSize: 17 }}>Science demos</span>
          <span className="pill green">pre-loaded, no wait</span>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: 14 }}>
          {CUSTOM_DEMO_TOPICS.map((t) => (
            <button
              key={t.topic}
              className="card choice"
              style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 8, padding: 18 }}
              disabled={busy}
              onClick={() => onStartCustom(t.topic)}
            >
              <span
                style={{
                  width: 46, height: 46, borderRadius: 14, display: 'grid', placeItems: 'center',
              ...
              <span style={{ fontWeight: 900, fontSize: 16.5 }}>{t.title}</span>
              <span style={{ fontWeight: 700, fontSize: 13, color: 'var(--ink-mid)', textAlign: 'left' }}>{t.blurb}</span>
              {busy && busyLabel === t.topic && <span style={{ fontSize: 12.5, fontWeight: 800, color: 'var(--blue-d)' }}>Preparing…</span>}
              {!busy && (
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12.5, fontWeight: 900, color: 'var(--green)' }}>
                  Start <ArrowRight size={13} />
                </span>
