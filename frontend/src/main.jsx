...
export default function App() {
  ...
  async function handleBack() {
    ...
    setError('');
    try {
      const s = await api.regress(token, session.session_id);
      setSession(s);
      if (s.session_status === 'visualize') {
        setScreen('visualize');
      } else if (s.session_status === 'practice') {
        setScreen('challenge');
      } else if (s.session_status === 'explain') {
        setScreen('lesson');
      } else {
        setScreen('home');
      }
  ...
  return (
    <div className="app-shell">
      ...
      <main className="app-body">
        ...
        {screen === 'lesson' && session && (
          <Lesson
            token={token}
            session={session}
            onContinue={() => setScreen('visualize')}
        ...
        {screen === 'challenge' && session && (
          <Challenge
            ...
            session={session}
            busy={busy}
            onSubmit={(answer) => nextStep(answer)}
            onBack={handleBack}
          />
        )}
        ...
        {screen === 'feedback' && session && (
          <Feedback
            session={session}
            busy={busy}
            onContinue={handleAdaptContinue}
            onBack={handleBack}
          />
        )}
